document.addEventListener("DOMContentLoaded", function() {
  ...
  modal.addEventListener("show.bs.modal", function(event) {
    ...
    fetch(`/file/archivist_preview/${fileId}`)
      .then((response) => {
        ...
        if (contentType.includes("image")) {
          ...
          modalContent.innerHTML = `<img src="/file/archivist_preview/${fileId}" alt="Image preview" style="max-width:60vw;max-height:40vw;"/>`;
        } else if (contentType.includes("audio")) {
          modalTitle.innerHTML = `${fileTitle}`;
          modalContent.innerHTML = `<audio controls style="justify-content:center;align-items:center;max-width:40vw;">
                                        <source src="/file/archivist_preview/${fileId}" type="${contentType}">
                                        Your browser does not support the audio element.
                                    </audio>`;
        } else if (contentType.includes("pdf")) {
          modalTitle.innerHTML = `${fileTitle}`;
          modalContent.innerHTML = `<iframe src="/file/archivist_preview/${fileId}" style="width:100%;height:40vw"></iframe>`;
